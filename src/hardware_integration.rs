...
use crate::quantum_backend::{
    BackendManager, HardwareCircuit, HardwareGate, QuantumConfig, QuantumResult,
};
...
impl HardwareExecutor {
    ...
    pub fn execute_on_hardware(
        ...
        println!(
            "📡 Submitting to {:?} with {} qubits and {} gates",
    ...
    fn record_statement(
        ...
        match node {
            ...
            | ASTNode::CircuitDeclaration { body, .. } => self.record_statement(body, env),

            _ => Ok(()), // Skip non-quantum statements
        }
    }

    fn record_gate_application(
        &mut self,
        gate_expr: &ASTNode,
        arguments: &[ASTNode],
    ) -> Result<(), String> {
        ...
        self.recorder
            .record_gate(&gate_name, qubit_indices, params, is_dagger);
        Ok(())
    }
...
pub fn parse_hardware_config(args: &[String]) -> Option<QuantumConfig> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--hardware" => {
                if i + 1 < args.len() {
                    ...
                    config.provider = match provider_str.as_str() {
                        ...
                        "native" | "simulator" => crate::quantum_backend::QuantumProvider::Native,
                        _ => {
                            eprintln!("Unknown provider: {}. Using native.", provider_str);
                            crate::quantum_backend::QuantumProvider::Native
                        }
                    };
                    i += 2; // Skip both --hardware and the provider name
                } else {
                    eprintln!("Error: --hardware requires a provider argument");
                    return None;
                }
            }
            "--device" => {
            ...
            "--shots" => {
                if i + 1 < args.len() {
                    match args[i + 1].parse::<u32>() {
                        Ok(shots) => config.shots = shots,
                        Err(_) => {
                            eprintln!("Error: Invalid shots value: {}", args[i + 1]);
                            return None;
                        }
                    }
                    i += 2;
                } else {
                    eprintln!("Error: --shots requires an argument");
                    return None;
                }
            }
            "--api-token" => {
                if i + 1 < args.len() {
                    ...
                    eprintln!("Error: --api-token requires an argument");
                    return None;
                }
            }
            "--no-optimize" => {
            ...
            _ => {
                // Skip unknown arguments (they might be filenames or other flags)
                i += 1;
            }
        }
    }
...
/// Extract filename from arguments (ignoring hardware flags)
pub fn extract_filename(args: &[String]) -> Option<String> {
    ...
    while i < args.len() {
        ...
        if arg == "--hardware" || arg == "--device" || arg == "--shots" || arg == "--api-token" {
            i += 2; // Skip flag and its value
            continue;
        }

        // Skip standalone flags
        if arg == "--no-optimize" || arg == "--list-devices"
            || arg == "--ast" || arg == "--tokens" || arg == "--emit-llvm"
            || arg == "--lto" || arg == "--v" {
            i += 1;
            continue;
        }
        ...
        if arg.starts_with("-O") || arg.starts_with("--target=") {
            i += 1;
            continue;
        }

        // Skip commands
        if arg == "--compile" || arg == "--run" || arg == "--doc"
            || arg == "--repl" || arg == "--test" || arg == "--lex" {
            i += 1;
            continue;
        }

        // If it doesn't start with -- and isn't a known flag, it's probably the filename
